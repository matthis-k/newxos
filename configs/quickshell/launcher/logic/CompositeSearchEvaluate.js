...
function evaluateNode(node, query, ctx) {
    ...
    if (selfAllowed && directCandidate) {
        ...
        for (var pi = 0; pi < evidencePolicies.length; pi += 1) {
            var policy = PolicyRegistry.evidence.get(evidencePolicies[pi]);
    ...
    // Check for base evidence (excludes usage/recency for the hasBase test)
    var hasBase = ownEvidence.some(function(e) { return e.field !== "usage" && e.field !== "recency"; });

    // ── Phase 2: Children ──
    var evaluatedChildren = evaluateChildren(node, query, ctx, directiveActive);

    // Score own evidence
    var own = selfAllowed ? scoreEvidence(ownEvidence, node, ctx) : { value: 0, visible: false, reason: "directive container only" };
    ...
    // Score inherited evidence (from phase 1 breadcrumb matches)
    var inheritedResult = selfAllowed && inheritedEvidence.length ? scoreEvidence(inheritedEvidence, node, ctx) : { value: 0 };
    ...
    for (var bi = 0; bi < boostPolicies.length; bi += 1) {
        var bpol = PolicyRegistry.boost.get(boostPolicies[bi]);
    ...
    var finalScore = clamp(Math.max(own.value, inheritedScore, descendantBoost));

    // action alias boost (separate from descendant boost, applied on top)
    var actionAliasBoost = 0;
    if (node.switchActions && own.value > 0) {
        var aliasPol = PolicyRegistry.boost.get("switch-aliases");
        if (aliasPol) {
...
// ── Inherit Phase Dispatch (called from pipeline after evaluateNode) ──
...
function applyInheritPolicies(evaluated, query, ctx) {
    ...
    for (var pi = 0; pi < inheritNames.length; pi += 1) {
        var policy = PolicyRegistry.inherit.get(inheritNames[pi]);
        if (policy && policy.phase === "inherit")
            policy.apply(evaluated, query, ctx);
    }
}

// ── Utility: hasBaseEvidence (used by flatten) ──
