        ...
        if (expression && looksLikeMath(expression)) {
            try {
                ...
                children.push(root.node({
                    ...
                    evaluationProfile: { mode: "generic+custom", strategies: ["exact", "prefix", "compact", "substring", "acronym", "semantic"], scorePolicy: "semantic-result", profile: { evidence: ["field-match:all", "semantic"], inherit: [], boost: [], childVisible: ["visible-flag", "above-min-score:0.25"], childBypass: ["own-score-beats-parent", "score-dominates:0.03"], tokenFlow: ["pass-all"], takeoverRequest: [], takeoverAccept: [], expand: ["expand-none"], retainParent: ["retain-always"], defaultAction: ["default-action-owner"], riskGate: ["risk-gate"] } },
