pragma ComponentBehavior: Bound

import QtQuick
import qs.services

ListView {
    id: root

    property var visualResults: null
    property Component resultDelegate: null
    property var controller: null
    property int iconSize: 32
    property int visibleResultRows: 12
    property bool showSubtitles: true
    property bool showActionHint: true
    property bool showEvidence: false
    property int rowSpacing: Config.spacing.xxs
    property int estimatedRowHeight: 56
    readonly property int activeAnimationMode: visualResults ? visualResults.animationMode : 2
    readonly property int enterDuration: durationForMode(140, 90)
    readonly property int removeDuration: durationForMode(120, 80)
    readonly property int moveDuration: durationForMode(140, 100)
    readonly property int transformDuration: durationForMode(120, 80)

    signal closeRequested()

    model: visualResults ? visualResults.model : null
    boundsBehavior: Flickable.StopAtBounds
    cacheBuffer: root.visibleResultRows * (root.estimatedRowHeight + root.rowSpacing)
    clip: true
    spacing: 0
    reuseItems: false

    function durationForMode(fullDuration, lightDuration) {
        if (!Config.behaviour.animation.enabled || root.activeAnimationMode === 0)
            return 0;
        return root.activeAnimationMode === 1 ? lightDuration : fullDuration;
    }

    function pinContentToTopIfNeeded() {
        if (contentHeight <= height || (!root.controller || !root.controller.isInTree || (!root.controller.isInTree() && count <= root.visibleResultRows)))
            contentY = 0;
    }

    function settleLayout() {
        forceLayout();
        pinContentToTopIfNeeded();
    }

    onCountChanged: Qt.callLater(settleLayout)
    onContentHeightChanged: Qt.callLater(settleLayout)
    onHeightChanged: Qt.callLater(settleLayout)

    move: Transition {
        NumberAnimation {
            properties: "x,y"
            duration: root.moveDuration
            easing.type: Easing.OutCubic
        }
    }

    moveDisplaced: Transition {
        NumberAnimation {
            properties: "x,y"
            duration: root.moveDuration
            easing.type: Easing.OutCubic
        }
    }

    delegate: AnimatedResultDelegate {
        sourceComponent: root.resultDelegate
        controller: root.controller
        animationMode: root.activeAnimationMode
        enterDuration: root.enterDuration
        removeDuration: root.removeDuration
        transformDuration: root.transformDuration
        spacing: root.rowSpacing
        estimatedRowHeight: root.estimatedRowHeight
        iconSize: root.iconSize
        showSubtitle: root.showSubtitles
        showActionHint: root.showActionHint
        showEvidence: root.showEvidence

        onActivated: function(rank) {
            if (!root.controller)
                return;
            root.controller.selectedIndex = rank;
            if (root.controller.activateSelected(false))
                root.closeRequested();
        }
    }
}
