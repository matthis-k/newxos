...
QtObject {
    ...
    function roots(context) { tracer.trace("roots", function() { return {}; }); return [{ id: "network", aliases: ["net", "network", "networking"], title: qsTr("Networking"), icon: "network-wireless-symbolic", iconColor: Config.styling.primaryAccent, template: "flat-action-group", behavior: { filterChildren: true }, evaluationProfile: EvalProfiles.EvaluationProfiles.groupProfile(), children: [
        ...
        { id: "vpn", aliases: ["vpn", "connect to"], title: qsTr("VPN"), icon: "network-vpn-symbolic", iconColor: VpnService.connected || VpnService.connecting ? Config.styling.good : Config.styling.warning, template: "switch", switchState: VpnService.connected || VpnService.connecting, behavior: { filterChildren: true, depthPenalty: 1, exploration: { descend: false } }, evaluationProfile: EvalProfiles.EvaluationProfiles.switchProfile(), switchActions: {
