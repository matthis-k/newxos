...
- [fact] This repo uses Basic Memory for semantic knowledge graph indexing across `knowledge/`
- [requirement] **Always lead with Basic Memory** for any repo knowledge query — before reading files, running commands, or making changes
- [technique] Search order is strict: Basic Memory search → `rg` over `knowledge/` → source inspection
- [decision] Knowledge uses structured folders with README.md indexes rather than flat files
...
- [requirement] Pick task-appropriate skills before working; for example use Qt/QML docs or QML skills when creating or changing QML
- [requirement] Do not read or print secrets unless explicitly asked; prefer checking paths, wiring, and permissions instead
- [fact] `flake.nix` is generated by `flake-file`; edit `modules/` not the generated file
...
**First step for any task:** Search Basic Memory for relevant knowledge before reading files or running commands.
...
- Load task-appropriate skills before specialized work, such as QML, Qt/C++, UI design, profiling, docs, or code review.
...
- Keep `knowledge/` focused on concept-to-file indirection, placement rules, durable decisions, recurring issues, and library references.
- Keep exact option values, package lists, generated settings, and minor local rationale in source files or nearby source comments instead of duplicating them in memory.
